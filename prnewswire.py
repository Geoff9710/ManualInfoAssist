# -*- coding: utf-8 -*-
"""
Created on Wed Aug  1 14:17:23 2018
...
def PR_newswire(Company):
    ...
    try:    
      html = urlopen('http://www.prnewswire.co.uk/news-releases/travel-latest-news/leisure-tourism-list/?page=1&pagesize=100').read()
    except:
      print('#1 PRnews1 url open fail')
    ...
    if soup is not None:
        try:
            ...
            print("#2 PRnews1 heading_html scraping error")
            
        try:
            para_html = soup.find('div', class_='col-sm-8 card-list-hr card-list').find_all('p')
        except:
            print("#3 PRnews1 para_html scraping error")
